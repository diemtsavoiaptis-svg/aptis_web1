...
for number, url in DATA.items():
    material = (
        ...
        .filter(part=4, description__strip=number)
        .first()
    )
    ...
    if material is None:
        material = (
            ListeningPartMaterial.objects
            .filter(part=4, description=number)
            .first()
        )

    if material is None:
        # fallback: tìm description có chứa đúng số đó
        material = (
            ListeningPartMaterial.objects
            .filter(part=4, description__icontains=number)
            .order_by("id")
            .first()
        )
    ...
    if material is None:
        ...
        print(f"MISSING ROW {number}: không tìm thấy dòng có Question={number}")
        continue

    material.audio_url = url
    material.save(update_fields=["audio_url"])
    updated += 1
    print(f"OK Question {number}: ID={material.id} | {material.title}")
...
print("VERIFY 16-54:")
for number in DATA:
    material = ListeningPartMaterial.objects.filter(part=4, description=number).first()
    print(f"{number}: {'OK' if material and material.audio_url else 'MISSING'}")
