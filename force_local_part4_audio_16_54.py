...
print("DATABASE USED BY LOCAL SERVER:")
...
print("Part 4 materials found:", len(materials))

if len(materials) < 54:
    raise SystemExit("ERROR: Local database has fewer than 54 Part 4 rows.")

for number, url in DATA.items():
    material = materials[number - 1]
    material.audio_url = url
    material.save(update_fields=["audio_url"])
    print(f"OK {number}: ID={material.id} | {material.title} | saved={bool(material.audio_url)}")

print()
print("VERIFY 16-54:")
for number, material in enumerate(materials[15:54], start=16):
    material.refresh_from_db()
    print(f"{number}. {'OK' if material.audio_url else 'MISSING'} | {material.title} | {material.audio_url[:45]}")

print("DONE: Local Part 4 audio 16-54 saved.")
