...
for path in targets:
    ...
    if "Part 3" in text and "Mở Part 2" in text:
        ...
        text = re.sub(
            r'(?is)(Part 3.*?)(Mở Part 2)',
            lambda m: m.group(1).replace('/dashboard/part-2/', '/dashboard/part-3/').replace('part-2', 'part-3') + 'Mở Part 3',
            text,
            count=1
        )

    if "Part 4" in text and "Mở Part 2" in text:
        # Sửa riêng các block có Part 4
        text = re.sub(
            r'(?is)(Part 4.*?)(Mở Part 2)',
            lambda m: m.group(1).replace('/dashboard/part-2/', '/dashboard/part-4/').replace('part-2', 'part-4') + 'Mở Part 4',
            text,
            count=1
        )

    # Sửa các href rõ ràng nếu nằm trong card Part 3/4 dạng ngắn
    text = text.replace('href="/dashboard/part-2/">Mở Part 3', 'href="/dashboard/part-3/">Mở Part 3')
    text = text.replace("href='/dashboard/part-2/'>Mở Part 3", "href='/dashboard/part-3/'>Mở Part 3")
    text = text.replace('href="/dashboard/part-2/">Mở Part 4', 'href="/dashboard/part-4/">Mở Part 4')
    text = text.replace("href='/dashboard/part-2/'>Mở Part 4", "href='/dashboard/part-4/'>Mở Part 4")
