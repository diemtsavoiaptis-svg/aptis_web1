...
print(f"Online Part 4 materials found: {len(materials)}")

if len(materials) < 54:
    raise SystemExit(f"ERROR: Need at least 54 Part 4 materials, found {len(materials)}")
...
with transaction.atomic(using="online"):
    for number, url in DATA.items():
        ...
        material.save(using="online", update_fields=["audio_url"])
        updated += 1
        print(f"OK {number}: ID={material.id} | {material.title}")

print()
print(f"DONE: Updated online Part 4 audio rows 16-54. Updated: {updated}")
